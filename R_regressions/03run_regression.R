...
#Create actual regression dataset with price specific variables and attmempts to filter out messy data

dwellings_with_prices <- read_csv("data/dwellings_with_prices.csv")

#get sa3 data which was deleted from the previous dataframe a little too early! 
...
fix_errant_sa3 <- function(data){
  ...
  #One sa3 only has 2 obs, let's just assign it to a nearby one
  output <- data %>% 
...
clean_data_set_for_regression <- function(x){
  ...
  output <- x %>% 
    lazy_dt() %>% 
    mutate(zone_short = if_else(zone_short == "Residential Growth","Residential growth",zone_short)) %>% 
    mutate(sa1_code_2021 = as.character(sa1_code_2021)) %>% 
    filter(!is.na(sa1_code_2021)) %>% # Only about 20 obs missing an sa1 because they're in weird locations - and they're mostly civid land. 
    ...
    mutate(across(all_of(c(walkability_metrics,"dist_rail","traffic_pollution")), ~replace_na(.x,mean(.x, na.rm = TRUE)))) %>%
    ungroup() %>% 
    mutate(across(all_of(c(walkability_metrics,"dist_rail")), ~replace_na(.x,5001))) %>% 
    mutate(across(c("dist_rail","lot_size","traffic_pollution",walkability_metrics,"cbd_dist"), ~winsorize(.x,threshold = .05),.names = "wins_{.col}")) %>% 
    mutate(across(paste0("wins_",c("dist_rail","lot_size","traffic_pollution",walkability_metrics,"cbd_dist")), ~log(.x),.names = "ln_{.col}")) %>% 
    mutate(across(paste0("wins_",c("dist_rail","lot_size","traffic_pollution",walkability_metrics,"cbd_dist")), ~1/(.x),.names = "inv_{.col}")) %>% 
    mutate(fct_lot_size = cut(lot_size, breaks = c(0, 300, 500, 750, 1000, 1250,Inf),right = F)) %>% 
    mutate(across(all_of(c(walkability_metrics,"dist_rail")), ~cut(.x, breaks = c(0,100,200,300,400,500,800,Inf),right = F), .names = "fct_{.col}")) %>% 
...
all_prices <- dwellings_with_prices %>% 
  ...
  mutate(year = fct_relevel(as.factor(year),"2010"),
         ...
         across(all_of(paste0("fct_",c(walkability_metrics,"dist_rail"))),~fct_relevel(.x, "[800,Inf)")),
...
house_dataset <- all_prices %>% 
  ...
  mutate(land_size = coalesce(land_size,lot_size)) %>%  # Exclude cases where sales data and us disagree on how big hte lot is (often data entry errors on the sales data)
  filter(between(land_size,lot_size*.8,
                 lot_size*1.2)) %>% 
  filter(lot_size <3000) %>% #Big lots are confusing as heck! 
  filter(dwlgs_in_2016 <2) %>%  # Exclude anything we think might be apartments despite sales data not suggesting that
  filter(!is.na(sale_price))

apartment_dataset <- all_prices %>% 
  filter(prop_type_short == "unit") %>% 
  filter(dwellings_est >6) %>% #Try and exlcude townhouses 
  filter(sale_price>250000,
         sale_price<2000000) %>% 
  filter(!is.na(sale_price)) %>% 
  mutate(dwellings_est = replace_na(dwellings_est,0)) %>% 
  filter(!is.na(wins_traffic_pollution)) %>%
  filter(!is.na(pp_bedrooms)) 
...
##IMPORT PREDICTIONSBACK INTO THE ORIGINAL DATASET (sorry this is a bit slow!)
