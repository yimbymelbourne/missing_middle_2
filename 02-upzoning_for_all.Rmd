...
## Upzoning across all of Melbourne
...
## Zoned capacity must exceed our housing need

Under the current system, the zoned capacity of each LGA is kept only barely higher than the number of homes that need to be built. Planners do this because if there is no choice then it creates much less uncertainty about what will be built, and so it is easier to design a city. 

But the limiting zoned capacity relies on the impractical idea that every potential lot will be built to its maximum zoned capacity within a reasonable timeframe. 
...
Planning in this way planning to limit zoned capacity has two key problems: 

1. it limits housing development, and
2. it encourages land speculation.

To ensure housing abundance, we must do better. 

When there isn't enough zoned capacity, it leads to land speculation and landbanking. When there are only a few plots of land where one can build, a small number of speculators can feasibly gain control of the bulk of land available and viable for development. However, if developable land is made abundantly available through broad upzoning, each land-holder would have much more competition and so it will be much harder to withold housing supply. 
