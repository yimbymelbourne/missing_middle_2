...
library(dtplyr)
apartment_size = 85
base_cost = 3800*apartment_size * 1.17*1.2 #figure here, plus 6% inflation since release, plus 30% developer profit https://content.knightfrank.com/research/911/documents/en/melbourne-new-apartments-insight-q3-2023-10663.pdf
...
price_estimates <- dwelling_data_raw %>% 
  ...
  mutate(property_price = property_price_per_sqm * lot_size,
         apartment_price = apartment_inflation * apartment_price)
...
sa2_prices <- price_estimates_dt %>% 
  ...
  summarise(price_mean           = mean(property_price_per_sqm, na.rm = TRUE),
            apartment_price_mean = mean(apartment_price, na.rm = TRUE),
            price_median = median(property_price_per_sqm, na.rm = TRUE),
            apartment_price_median = median(apartment_price, na.rm = TRUE))
...
price_estimates_dt %>% 
  ...
  mutate(property_price_per_sqm           = if_else(is.na(property_price_per_sqm), 1, 0),
         apartment_price                 = if_else(is.na(apartment_price), 0, 1)) %>% 
    group_by(zone_short,
  ...
  summarise(`Share with a house price estimate`      = round(100*mean(property_price_per_sqm)),
            `Share with an apartment price estimate` = round(100*mean(apartment_price))) %>% 
  as_tibble() %>% 
  DT::datatable()
...
median_price = prettyNum(median(price_estimates_dt$apartment_price,na.rm = TRUE),big.mark = ",")
mean_price = prettyNum(mean(price_estimates_dt$apartment_price,na.rm = TRUE),big.mark = ",",)
```
...
```{r apartment_price, echo=FALSE}

negative_values <- price_estimates_dt %>% 
  filter(apartment_price<0) %>% 
  nrow()
price_estimates_dt %>% 
  filter(apartment_price>=0)  %>% 
  ggplot(aes(x = apartment_price))+
  geom_histogram()+
  theme_minimal()+
...
filter(property_price >0, apartment_price >0) %>% 
  select(lot_size,apartment_price,property_price,zone_short,cbd_dist,prox_dist_m_tram,prox_dist_m_train) %>% 
...
mutate(profit_per_apartment = (apartments*(apartment_price-base_cost) - property_price - demolition_costs)/apartments,
