# Housing Targets for Banyule

```{r run-all-banyule-code}

lga_level_data <- run_for_area ("Banyule")

```

In `r lga_level_data$area_name`, `r lga_level_data$mm_number` lots would be upzoned to the Missing Middle Zone (MMZ), enabling six-storey medium-density near all transit. A further `r lga_level_data$rgz_number` properties will be rezoned from General Residential Zone (GRZ) to Residential Growth (RGZ), and `r lga_level_data$grz_number` from Neighbourhood Residential Zone (NRZ) to GRZ. 
...
## Upzoning `r lga_level_data$area_name`  {.tabset}
...
There are `r parcels` parcels of land in `r lga_level_data$area_name`. But not every piece of land can have housing on it. Of these parcels: 
...
YIMBY Melbourne's plan for `r lga_level_data$area_name` increases its zoned capacity by `r lga_level_data$zoned_capacity_x`, to `r prettyNum(round(lga_summary$mm_zoned_capacity,-3), big.mark = ",")` — enabling many more opportunities for dense housing to be built across the entire city.
