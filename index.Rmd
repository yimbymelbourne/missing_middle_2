...
tbl_body  <- lga_zoning_numbers %>% 
  select(`Council`        = lga_name_2022,
         ...
         `Existing home density (per square km)`= existing_dwelling_density,
         `Current yearly growth in homes (from 2016-2022)` = `Current yearly growth in homes (from 2016-2022)`,
         `Yearly housing target` = mm_target,
         `Change in pace of building needed to meet target` = change_to_target_required) %>% 
  arrange(desc(`Change in pace of building needed to meet target`))

total = tbl_body %>% 
  summarise(across(any_of(c("Existing homes",
                            "Current yearly growth in homes (from 2016-2022)",
                            "Yearly housing target")), sum)) %>%
  mutate(`Existing home density (per square km)` = `Existing homes`/sum(lga_zoning_numbers$areasqkm_2021),
        `Change in pace of building needed to meet target` = `Yearly housing target`/`Current yearly growth in homes (from 2016-2022)`,
        `Council` = "Total") %>%
  relocate(names(tbl_body)) %>% 
  mutate(across(c("Existing homes",
                  "Current yearly growth in homes (from 2016-2022)",
                  "Existing home density (per square km)",
                  "Yearly housing target"),~prettyNum(round(.x,-2),big.mark = ",")),
         `Change in pace of building needed to meet target` = paste0(round(100*`Change in pace of building needed to meet target`),"%") )
...
tbl_body  %>% 
  mutate(across(c("Existing homes",
                  "Current yearly growth in homes (from 2016-2022)",
                  "Existing home density (per square km)",
                  "Yearly housing target"),~round(.x,-2)))%>%
  datatable(
...
Each LGA's target has been carefully calculated based on real-world information. The full target calculation methodology is available [later in this report](implementing-housing-targets.html#calculating-targets).
