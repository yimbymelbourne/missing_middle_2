...
tbl_body  <- lga_zoning_numbers %>% 
  select(`Council` = lga_name_2022,
         ...
         `New zoned capacity` = mm_zoned_capacity,
...
total = tbl_body %>% 
  summarise(`Current zoned capacity` = sum(`Current zoned capacity`),
            ...
            `New zoned capacity` = sum(`New zoned capacity`),
            `Yearly housing target` = sum(`Yearly housing target`)) %>% 
  mutate(`Increase in zoned capacity` = `New zoned capacity`/`Current zoned capacity`,
         `Council` = "Total")
...
footer_values <- c(
  ...
  prettyNum(round(total$`Current zoned capacity`, -3), big.mark = ","),
  prettyNum(round(total$`New zoned capacity`, -3), big.mark = ","),
...
tbl_body %>%
  mutate(across(c(`Current zoned capacity`, `New zoned capacity`), ~ round(.x, -3)),
